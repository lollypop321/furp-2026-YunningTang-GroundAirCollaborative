...
#     # 按延迟排序，优先搜索延迟低的解
#     search_points = sorted(archive, key=lambda x: x[1])[:5]
...
#             # ===== 第一步：2-opt优化 =====
#             for tid in range(TRUCK_NUM):
#                 new_routes = operator_2opt_enhanced(routes, tid, max_iter=5)
...
#                     delay_change = new_delay - current_delay
                    
#                     # ===== 接受准则 =====
#                     accept = False
                    
#                     # 1. 延迟改善：无条件接受
#                     if delay_change < -1e-6:
#                         accept = True
#                     # 2. 延迟相同，成本降低：接受
#                     elif abs(delay_change) < 1e-6 and cost_change < 0:
#                         accept = True
#                     # 3. 延迟略有增加(≤2)，但成本显著降低(≥10)：接受
#                     elif delay_change <= 2.0 and cost_change <= -10.0:
#                         accept = True
#                     # 4. 延迟增加不超过5，但成本降低超过20
#                     elif delay_change <= 5.0 and cost_change <= -20.0:
#                         accept = True
...
#         if all_feasible:
#             # ===== 综合目标排序（延迟权重1.5） =====
#             cur_best = min(all_feasible, key=lambda x: x[0] + 1.5 * x[1])
#             if best_feasible is None or (cur_best[0] + 1.5 * cur_best[1]) < (best_feasible[0] + 1.5 * best_feasible[1]):
#                 best_feasible = cur_best
...
#             if pareto_archive:
#                 best_point = min(pareto_archive, key=lambda x: x[0] + 1.5 * x[1])
#                 key = (round(best_point[0], 4), round(best_point[1], 4))
#                 if key in full_solutions:
#                     rt, da, te, de = full_solutions[key]
#                     best_feasible = (best_point[0], best_point[1], rt, da, te, de, None)
...
#             best_cost, best_delay, best_routes, best_drone_assigns, _, _, _ = best_feasible
#             # ===== 延迟权重提高（1.5倍） =====
#             best_delta = Qc / (best_cost + 1e-6) + 1.5 * Qt / (best_delay + 1e-6)
...
#         # 帕累托解贡献辅助信息素
#         if len(pareto_archive) > 1:
#             sorted_pareto = sorted(pareto_archive, key=lambda x: x[0] + 1.5 * x[1])
#             for idx, (cost, tard) in enumerate(sorted_pareto[:5]):
#                 weight = 1.0 - idx * 0.15
#                 delta = (Qc / (cost + 1e-6) + 1.5 * Qt / (tard + 1e-6)) * weight
#                 key = (round(cost, 4), round(tard, 4))
#                 if key in full_solutions:
#                     rt, da, te, de = full_solutions[key]
#                     for route in rt:
