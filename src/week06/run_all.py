...
#                     'min_cost': result.get('min_cost', result['best_cost']),
#                     'min_delay': result.get('min_delay', result['best_delay']),
#                     'pareto_size': result['pareto_size'],
#                     'runtime': elapsed,
#                 })
#                 print(f"    ✅ 综合={result['best_objective']:.2f}, 点数={result['pareto_size']}")
...
#         # 保存单个版本结果
#         df.to_csv(f"../results/{name}/results.csv", index=False)
#         print(f"✅ {name} 结果已保存到 ../results/{name}/results.csv")
    
#     # ===== 合并对比 =====
#     if all_dfs and all(not df.empty for df in all_dfs):
...
#         # 按规模和版本分组统计
#         summary = combined.groupby(['scale', 'version']).agg({
...
#         print("\n✅ 汇总结果已保存到 ../results/summary.csv")
#         print("✅ 所有结果已保存到 ../results/all_results.csv")
#     else:
#         print("\n❌ 没有成功的结果可以汇总")
...
# def generate_comparison_plots(df, save_dir="results"):
#     """生成对比图表"""
#     if df.empty:
#         print("❌ 没有数据")
#         return
...
#     # 过滤有效数据
#     df_valid = df[df['best_objective'].notna()].copy()
#     if df_valid.empty:
#         print("❌ 没有有效数据")
#         return
...
#             data = df_valid[(df_valid['scale'] == scale) & (df_valid['version'] == version)]['best_objective'].dropna()
#             if len(data) > 0:
...
#     scale_labels = []
#     for scale in scales:
#         orig_data = df_valid[(df_valid['scale'] == scale) & (df_valid['version'] == 'original')]['best_objective']
#         pls_data = df_valid[(df_valid['scale'] == scale) & (df_valid['version'] == 'with_pls')]['best_objective']
...
#         means = data.groupby('scale')['best_objective'].mean()
#         stds = data.groupby('scale')['best_objective'].std()
#         if len(means) > 0:
#             ax.errorbar(means.index, means.values, yerr=stds.values,
#                         capsize=5, marker=marker, color=color, label=version, linewidth=2, markersize=8)
#     ax.set_xlabel('客户数量')
#     ax.set_ylabel('平均综合目标')
#     ax.legend()
#     ax.set_title('不同规模下两个版本对比')
#     ax.grid(True)
...
#     ax.set_xlabel('客户数量')
#     ax.set_ylabel('平均帕累托点数')
#     ax.legend()
#     ax.set_title('帕累托点数对比')
#     ax.grid(True)
