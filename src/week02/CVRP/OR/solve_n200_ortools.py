##########################################################################################
# OR-Tools Solver for CVRP with EV and TW - n=200
##########################################################################################
...
def solve_with_ortools(distance_matrix, demands, tw_early, tw_late, service_time,
    ...
    if use_tw and tw_early is not None and tw_late is not None:
        ...
        routing.AddDimension(time_callback_index, 0, int(20 * 1000), False, 'Time')
...
def main():
    ...
    for i in range(N_INSTANCES):
        ...
        if (i + 1) % 10 == 0:
            ...
            print(f"Instance {i+1}/{N_INSTANCES}: Cost={cost:.4f if cost else 'N/A'}, Avg={avg_cost:.4f}")
