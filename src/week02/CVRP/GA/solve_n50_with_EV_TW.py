##########################################################################################
# GA Solver for CVRP with EV and TW - n=50
##########################################################################################
...
def solve_single_instance(problem_data, instance_id):
    ...
    if len(best_ind) != PROBLEM_SIZE or len(set(best_ind)) != PROBLEM_SIZE:
        print(f"  Warning: Instance {instance_id} invalid individual! len={len(best_ind)}, unique={len(set(best_ind))}")
...
def main():
    ...
    print(f"Crossover: {CX_PB}, Mutation: {MUT_PB}")
